...
import ExcitingBilibili.Exception.{CommentForbiddenException, ParseCommentException, VideoNotExistException}
...
private case class CommentPage(results: Int,
                               page: Int,
                               pages: Int,
                               isAdmin: Int,
                               needCode: Boolean,
...
object Comment {
  ...
  @throws(classOf[CommentForbiddenException])
    ...
    decode[CommentPage](jsonString) match {
      ...
      case Left(error) =>
        decode[ErrorCommentResponse](jsonString) match {
          case Right(errorInfo) => errorInfo.code match {
            case 12002 => throw CommentForbiddenException(av)
            case -404 => throw VideoNotExistException(av)
          }
